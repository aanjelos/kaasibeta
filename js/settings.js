  ...
  if (!accountManagementList) {
    ...
    state.accounts.forEach((acc) => {
      ...
      if (acc.id === "cash") {
        ...
        nameInput.classList.add("text-neutral-400", "cursor-not-allowed");
      }
...
function renderCategorySettingsList() {
  ...
  sortedCategories.forEach((cat) => {
    ...
    const eyeIconClass = isHidden ? "fa-eye-slash text-neutral-500" : "fa-eye text-accent-primary";
    ...
    const deleteButtonHTML = `<button class="text-neutral-400 hover:text-expense focus:outline-none w-6 h-6 flex items-center justify-center" onclick="deleteCategory('${cat}')" data-tooltip="Delete Category"><i class="fas fa-times"></i></button>`;
...
function buildBudgetCategoryCheckboxes(containerId, selectedCategories = [], currentBudgetId = null) {
  ...
  if (generalCategories.length === 0) {
    container.innerHTML = `<p class="text-xs text-neutral-400 p-2">No categories available.</p>`;
    return;
  }
  ...
  generalCategories.forEach((cat) => {
    ...
    if (isUsed) {
      label.className = "flex items-center gap-2 px-2 py-1.5 opacity-40 cursor-not-allowed rounded text-sm text-neutral-400 transition-colors select-none";
    } else {
      label.className = "flex items-center gap-2 px-2 py-1.5 hover:bg-white/5 focus:bg-white/10 focus:outline-none cursor-pointer rounded text-sm text-neutral-300 transition-colors";
      label.tabIndex = 0;
    ...
    label.innerHTML = `
      ...
      <div class="w-4 h-4 rounded border ${isUsed ? "border-neutral-700 bg-neutral-800" : "border-neutral-500 peer-checked:border-accent-500"} flex items-center justify-center transition-colors text-transparent peer-checked:text-accent-500">
      ...
      ${isUsed ? `<span class="text-[10px] bg-neutral-850 text-neutral-400 px-1.5 py-0.5 rounded uppercase font-semibold tracking-wider">Used</span>` : ""}
    `;
...
function renderBudgetsSettingsList() {
  ...
  if (!state.budgets || state.budgets.length === 0) {
    container.innerHTML = `<p class="text-xs text-neutral-400">No active budgets.</p>`;
    return;
  }
  ...
  const updateFn = () => {
    ...
    sortedBudgets.forEach((budget) => {
      ...
      div.className = "flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 bg-black/20 rounded-md border border-neutral-600 gap-3";
    ...
    div.innerHTML = `
      <div class="flex-grow min-w-0">
        <div class="flex items-center justify-between sm:justify-start gap-2 mb-1">
          <h4 class="font-medium text-neutral-200 truncate">${budget.name}</h4>
          <span class="text-sm font-semibold text-accent-500 whitespace-nowrap">${formatCurrency(budget.limit)}</span>
        </div>
        <p class="text-xs text-neutral-400 line-clamp-2" data-tooltip="${catList}">Categories: ${catList || 'None'}</p>
      </div>
