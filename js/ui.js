...
function openShortcutsHelpModal() {
  ...
  shortcuts.forEach((shortcut) => {
    ...
    li.className =
      "flex justify-between items-center py-2 px-1 border-b border-[var(--border-color)] last:border-b-0";
    ...
    actionSpan.className = "text-[var(--text-secondary)] text-sm text-left flex-grow px-2"; // Action takes up most space
    actionSpan.textContent = shortcut.action;

    const contextSpan = document.createElement("span");
    contextSpan.className = "text-xs text-[var(--text-placeholder)] text-right w-1/4 italic"; // Context takes up roughly 1/4
...
function openCashCounter() {
  ...
  denominations.forEach((denom) => {
    ...
    inputEl.className =
      "text-center bg-[var(--bg-tertiary)] border border-[var(--border-color)] rounded px-1 py-0.5 w-16 mx-auto text-sm";
    ...
    totalEl.className = "text-right text-[var(--text-secondary)] text-sm";
