...
function openInitialSetupWizard() {
  ...
  const renderSetupAccounts = () => {
    ...
    tempSetupAccounts.forEach((acc) => {
      ...
      if (acc.id === "cash") {
        nameWrapper.innerHTML = `<label for="setupBalance-${acc.id}" class="text-sm font-medium text-neutral-300 justify-self-start">${acc.name}</label>`;
      } else {
...
function populateDropdowns() {
  ...
  if (filterDropdownMenu) {
    ...
    allLabel.className = "group flex items-center gap-3 px-2 py-2 hover:bg-white/5 focus:bg-white/10 focus:outline-none cursor-pointer rounded text-sm text-neutral-200 border-b border-neutral-700/50 pb-2 mb-1 transition-colors";
    allLabel.tabIndex = 0;
    ...
    allLabel.innerHTML = `
      ...
      <div class="w-4 h-4 rounded border border-neutral-500 peer-checked:border-accent-500 flex items-center justify-center transition-colors text-transparent peer-checked:text-accent-500">
    ...
    generalCategories.forEach((c) => {
      ...
      label.className = "group flex items-center gap-3 px-2 py-1.5 hover:bg-white/5 focus:bg-white/10 focus:outline-none cursor-pointer rounded text-sm text-neutral-300 transition-colors";
      label.tabIndex = 0;
