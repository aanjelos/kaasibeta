...
function renderRecentTransactions() {
  ...
  if (recent.length === 0) {
    list.innerHTML = '<div class="flex flex-col items-center justify-center py-8 text-[var(--text-placeholder)]"><i class="fas fa-receipt text-3xl mb-3 opacity-40"></i><p class="text-sm font-medium">No transactions yet</p></div>';
    return;
  }
  ...
  recent.forEach((t) => {
    ...
    div.dataset.id = t.id;
    div.className = `transaction-list-item-layout flex justify-between items-center rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-color)]/50 text-sm transition-all duration-200 hover:bg-[var(--bg-tertiary)] border border-[var(--border-color)]/70 hover:-translate-y-0.5 hover:shadow-md cursor-pointer group`;
    ...
    div.innerHTML = `
      <div class="flex-grow mr-2 overflow-hidden">
        ...
        <p class="text-xs text-[var(--text-secondary)]">${subDetailText}</p>
      </div>
      ...
      <div class="edit-btn-container flex-shrink-0 hidden md:flex">
        ...
        <button class="text-xs text-[var(--text-placeholder)] hover:text-expense focus:outline-none" onclick="deleteTransaction('${t.id}',event)" data-tooltip="Delete"><i class="fas fa-times"></i></button>
      </div>`;
...
function renderDebtList() {
  ...
  if (state.debts.length === 0) {
    listContainer.innerHTML =
      '<div class="flex flex-col items-center justify-center py-8 text-[var(--text-placeholder)]"><i class="fas fa-handshake text-4xl mb-3 opacity-40"></i><p class="text-sm font-medium">No debts recorded</p></div>';
    return;
  }
  ...
  sortedCreditors.forEach((creditorName) => {
    ...
    creditorWrapper.className =
      "mb-3 border border-[var(--border-color)] rounded-md overflow-hidden shadow-sm";

    const creditorHeader = document.createElement("div");
    creditorHeader.className =
      "flex flex-col sm:flex-row sm:justify-between sm:items-center items-start p-3 cursor-pointer hover:bg-[var(--bg-tertiary)] border border-[var(--border-color)]/50 transition-colors gap-2 sm:gap-0";
    ...
    creditorHeader.innerHTML = ` 
      ...
      <div class="flex items-center flex-shrink-0 w-full sm:w-auto justify-between sm:justify-end">
        ...
        <span class="toggle-icon text-[var(--text-secondary)]"><i class="fas fa-chevron-down text-xs"></i></span>
      </div>
    `;
    ...
    creditorData.items
      ...
      .forEach((d) => {
        ...
        if (daysLeft < 0) {
          ...
          daysColor = "text-[var(--text-secondary)]";
        }
        ...
        if (percentagePaid > 0) {
          progressBarHtml = `
            <div class="w-full bg-[var(--bg-tertiary)] border border-[var(--border-color)] rounded-full h-1.5 mt-2 mb-1" data-tooltip="${percentagePaid.toFixed(1)}% Paid">
        ...
        itemDiv.className =
          "transaction-list-item-layout text-sm border-b border-[var(--border-color)] last:border-b-0";
        itemDiv.innerHTML = `
          ...
          ${progressBarHtml}
          <div class="flex justify-between items-center text-xs text-[var(--text-placeholder)] mt-1">
            ...
            <div class="edit-btn-container">
              ...
              <button class="text-[var(--text-placeholder)] hover:text-expense text-xs focus:outline-none" onclick="deleteDebt('${
                d.id
...
function renderReceivableList() {
  ...
  if (state.receivables.length === 0) {
    listContainer.innerHTML =
      '<div class="flex flex-col items-center justify-center py-8 text-[var(--text-placeholder)]"><i class="fas fa-hand-holding-usd text-4xl mb-3 opacity-40"></i><p class="text-sm font-medium">No receivables recorded</p></div>';
    return;
  }
  ...
  const renderGroupInModal = (title, receivablesForGroup) => {
    ...
    sectionTitleHeader.className =
      "flex flex-col sm:flex-row sm:justify-between sm:items-end items-start border-b border-[var(--border-color)] pb-2 mb-3 gap-1";
    ...
    sortedPeople.forEach((personName) => {
      ...
      personData.items
        ...
        .forEach((r) => {
          ...
          if (percentagePaid > 0) {
            progressBarHtml = `
              <div class="w-full bg-[var(--bg-tertiary)] border border-[var(--border-color)] rounded-full h-1.5 mt-2 mb-1" data-tooltip="${percentagePaid.toFixed(1)}% Received">
                <div class="bg-income h-1.5 rounded-full transition-all duration-500" style="width: ${percentagePaid}%"></div>
              </div>
            `;
          }
          ...
          <div class="flex justify-between items-start mb-1 gap-x-2">
            <div class="flex-grow">
              ...
              <p class="text-xs text-[var(--text-secondary)]">${srcTxt}</p>
            </div>
          ...
          <div class="flex justify-between items-center text-xs text-[var(--text-placeholder)] mt-1">
            ...
            <div class="edit-btn-container">
              ...
              <button class="text-[var(--text-placeholder)] hover:text-expense text-xs focus:outline-none" onclick="deleteReceivable('${
                r.id
              }')" data-tooltip="Delete"><i class="fas fa-times"></i></button>
            </div>
          </div>
...
function renderInstallmentList() {
  ...
  if (sortedInstallments.length === 0) {
    list.innerHTML = '<div class="flex flex-col items-center justify-center py-8 text-[var(--text-placeholder)]"><i class="fas fa-calendar-check text-4xl mb-3 opacity-40"></i><p class="text-sm font-medium">No installments recorded</p></div>';
    return;
  }
  ...
  sortedInstallments.forEach((i) => {
    ...
    let daysLeftText =
      daysLeft < 0
        ? `<span class="text-[var(--text-placeholder)]">Finished</span>`
    ...
    const div = document.createElement("div");
    div.className = "transaction-list-item-layout rounded-lg bg-[var(--bg-tertiary)] border border-[var(--border-color)]/50 text-sm mb-2 transition-all duration-200 hover:bg-[var(--bg-tertiary)] border border-[var(--border-color)]/70 hover:-translate-y-0.5 hover:shadow-md cursor-pointer";
        ...
        <div class="edit-btn-container flex items-center justify-end gap-x-4">
            ${
              i.monthsLeft > 0
                ...
                <button class="text-sm text-[var(--text-secondary)] hover:text-gray-200 focus:outline-none transition-colors" onclick="openEditInstallmentForm('${i.id}')" data-tooltip="Edit"><i class="fas fa-edit"></i></button>
              `
                : `
                <button class="text-sm text-[var(--text-secondary)] hover:text-gray-200 focus:outline-none transition-colors" onclick="openEditInstallmentForm('${i.id}')" data-tooltip="Edit"><i class="fas fa-edit"></i></button>
              `
            }
            <button class="text-sm text-[var(--text-placeholder)] hover:text-expense focus:outline-none transition-colors" onclick="deleteInstallment('${
              i.id
            }')" data-tooltip="Delete"><i class="fas fa-times"></i></button>
        </div>
    `;
      ...
      <div class="hidden md:flex items-center gap-x-3 w-full">
          ...
          <div class="flex-grow flex justify-between items-start ml-2 min-w-0">
              <div class="flex flex-col min-w-0 pr-2">
                  ...
                  <p class="text-[11px] md:text-xs text-[var(--text-secondary)] mb-1.5 tabular-nums truncate">${formatCurrency(i.monthlyAmount)} / month</p>
                  <p class="text-[11px] md:text-xs text-[var(--text-placeholder)] truncate tabular-nums">${i.monthsLeft} of ${i.totalMonths} months left (${daysLeftText})</p>
              </div>
      ...
      <div class="flex md:hidden flex-col w-full">
          <div class="flex items-center gap-x-3 w-full">
              ...
              <div class="flex flex-col min-w-0 flex-grow">
                  ...
                  <p class="text-[11px] text-[var(--text-secondary)] mb-1.5 tabular-nums truncate">${formatCurrency(i.monthlyAmount)} / month</p>
                  <p class="text-[11px] text-[var(--text-placeholder)] truncate tabular-nums">${i.monthsLeft} of ${i.totalMonths} months left (${daysLeftText})</p>
              </div>
          </div>
          <div class="flex flex-row justify-between items-center w-full border-t border-[var(--border-color)] pt-3 mt-3">
...
function openTransactionDetailModal(transactionId, skipHistory = false) {
  ...
  const html = `
    <div class="text-center mb-4">
      ...
      <div class="text-sm text-[var(--text-secondary)] mb-3">${transaction.type.charAt(0).toUpperCase() + transaction.type.slice(1)}</div>
      <div class="text-lg font-semibold text-gray-50 px-2" style="text-wrap: balance;">${transaction.description}</div>
    </div>
    
    <div class="bg-[var(--bg-tertiary)] border border-[var(--border-color)]/30 rounded-lg p-4 space-y-2 mb-6">
      <div class="flex justify-between items-center py-2 border-b border-[var(--border-color)]/50">
...
function openCcTransactionDetailModal(transactionId, skipHistory = false) {
  ...
  const html = `
    <div class="text-center mb-4">
      <div class="text-3xl font-bold ${transaction.paidOff ? "text-[var(--text-placeholder)]" : "text-gray-200"} tabular-nums mb-1">${formatCurrency(transaction.amount)}</div>
    ...
    <div class="bg-[var(--bg-tertiary)] border border-[var(--border-color)]/30 rounded-lg p-4 space-y-2 mb-6">
      ...
      <div class="flex justify-between items-center py-2 border-b border-[var(--border-color)]/50">
        <span class="text-[var(--text-secondary)]">Paid So Far</span>
        <span class="font-medium text-[var(--text-secondary)]">${formatCurrency(transaction.paidAmount || 0)}</span>
      </div>
      <div class="flex justify-between items-center py-2 border-b border-[var(--border-color)]/50">
        <span class="text-[var(--text-secondary)]">Remaining</span>
        <span class="font-medium ${remainingOnItem > 0.005 ? "text-expense" : "text-[var(--text-secondary)]"}">${formatCurrency(remainingOnItem)}</span>
      </div>
    </div>
  ...
  const formHtml = `
            ...
            <div>
                ...
                <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="modalAmount" name="amount" value="${transaction.amount.toFixed(
                  2
                )}" step="0.01" min="0" placeholder="e.g., 1500.50" required><button type="button" class="calc-toggle-btn absolute right-4 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div>
            </div>
            <div>
...
function setupMonthlyView() {
  ...
  yearSelector.addEventListener("change", () => {
    ...
    $("#monthlyDetailsContainer").innerHTML =
      '<p class="text-center text-[var(--text-secondary)]">Select a month to view details.</p>';
...
function renderMonthlyDetails(
  ...
  summaryGrid.innerHTML = `
      <div class="monthly-view-summary-card"><p class="text-sm text-[var(--text-secondary)] mb-1">Total Income</p><p class="text-xl font-semibold text-income tabular-nums">${formatCurrency(
        totalIncome
      )}</p></div>
      <div class="monthly-view-summary-card"><p class="text-sm text-[var(--text-secondary)] mb-1">Total Expenses ${monthSpendingIndicatorHtml}</p><p class="text-xl font-semibold text-expense tabular-nums">${formatCurrency(
    totalExpense
  )}</p></div>
      <div class="monthly-view-summary-card"><p class="text-sm text-[var(--text-secondary)] mb-1">Net Flow</p><p class="text-xl font-semibold ${
  ...
  if (transactionsToDisplay.length === 0) {
    ...
    noTransactionsDiv.innerHTML = `<div class="flex flex-col items-center justify-center py-12 text-[var(--text-placeholder)]"><i class="fas fa-search text-4xl mb-3 opacity-40"></i><p class="text-sm font-medium">${msg}</p></div>`;
    ...
    sortedDays.forEach((dayData) => {
      ...
      chevronIcon.className = "fas text-xs text-[var(--text-secondary)]";
        ...
        .forEach((t, index) => {
          ...
          itemDiv.innerHTML = `
              <div class="flex-grow min-w-0 ${opacityClass} mr-2">
                <p class="font-medium truncate ${textColorClass}">${escapeHTML(t.description)}</p>
                <p class="text-xs text-[var(--text-secondary)] mt-0.5 truncate">${subDetailText}</p>
              </div>
              ...
              isIncome ? "+" : "-"
                ...
                <div class="edit-btn-container ml-3 hidden md:flex">
                  ...
                  <button class="text-xs text-[var(--text-placeholder)] hover:text-expense focus:outline-none ml-2" onclick="deleteTransaction('${
                    t.id
  ...
  if (sortedCategories.length > 0) {
    sortedCategories.forEach(([category, amount]) => {
      ...
      categoryList.innerHTML = '<li class="flex flex-col items-center justify-center py-8 text-[var(--text-placeholder)]"><i class="fas fa-folder-open text-3xl mb-3 opacity-40"></i><p class="text-sm font-medium">No expenses this month</p></li>';
  }
  ...
  if (sortedCategories.length > 0) {
    if (existingChartCard) {
      ...
      centerTitle.className = "text-sm text-[var(--text-secondary)] mb-1 truncate w-full";
      ...
      placeholderContainer.className = "flex flex-col items-center justify-center text-[var(--text-placeholder)] w-full";
    ...
    noChartCard.innerHTML =
      ...
      '<div class="flex flex-col items-center justify-center flex-grow text-[var(--text-placeholder)]"><i class="fas fa-chart-pie text-5xl mb-4 opacity-30"></i><p class="text-sm font-medium">No expense data for chart</p></div>';
...
function renderCreditCardSection() {
  ...
  if (progressBar) {
    if (limit <= 0) {
      ...
      progressBar.className = "bg-[var(--bg-secondary)] border border-[var(--border-color)] h-1.5 rounded-full transition-all duration-500 ease-out";
    } else {
...
function openCcHistoryModal() {
  ...
  const renderFilteredCcList = () => {
    ...
    if (filteredTransactions.length === 0) {
      listContainer.innerHTML = `<div class="flex flex-col items-center justify-center py-12 text-[var(--text-placeholder)]"><i class="fas fa-search text-4xl mb-3 opacity-40"></i><p class="text-sm font-medium">No transactions found for this period</p></div>`;
      ...
      filteredTransactions.forEach((t, index) => {
        ...
        if (ccHistoryFilter === "unpaid") {
          ...
          if (monthYearStr !== lastMonthYearStr) {
            ...
            headerDiv.className = "text-[var(--text-secondary)] font-bold text-sm mt-4 mb-0 pb-2 border-b border-[var(--border-color)] px-2";
        ...
        if (t.paidOff || remainingOnItem <= 0.005) {
          ...
          bigAmountText = `
            <span class="font-semibold text-sm sm:text-base tabular-nums text-[var(--text-placeholder)]">${formatCurrency(t.amount)}</span>
          `;
        } else if (t.paidAmount > 0) {
          statusText = `<span class="text-[var(--text-secondary)]">Paid ${formatCurrency(t.paidAmount)}</span>`;
          bigAmountText = `
            <span class="font-semibold sm:text-base tabular-nums hidden sm:inline text-expense">
              ${formatCurrency(remainingOnItem)} <span class="text-[var(--text-placeholder)] text-sm font-normal">of ${formatCurrency(t.amount)} Left</span>
            </span>
          ...
          statusText = `<span class="text-[var(--text-secondary)]">Unpaid</span>`;
          bigAmountText = `
            <span class="font-semibold sm:text-base tabular-nums hidden sm:inline text-expense">${formatCurrency(remainingOnItem)} <span class="text-[var(--text-placeholder)] text-sm font-normal">Left</span></span>
        ...
        if (isSelectable) {
          checkboxHtml = `
            <div class="flex-shrink-0 mr-3 flex items-center h-full" onclick="event.stopPropagation(); toggleCcItemSelection('${t.id}')">
              <div id="cc-checkbox-${t.id}" class="w-5 h-5 rounded-full border border-[var(--border-color)] flex items-center justify-center transition-colors cursor-pointer hover:border-accent-500">
        ...
        mainRow.innerHTML = `
          ...
          <div class="flex-grow min-w-0 mr-2 text-left">
              <p class="font-medium truncate ${t.paidOff ? "text-[var(--text-placeholder)]" : "text-gray-200"}">${escapeHTML(t.description)}</p>
              <p class="text-xs text-[var(--text-secondary)] mt-0.5 truncate">${formattedDate} &bull; ${statusText}</p>
          </div>
      ...
      if (filteredTransactions.length > 0) {
        ...
        if (ccHistoryFilter === "unpaid") {
          footerDiv.innerHTML = `<span class="text-[var(--text-secondary)] font-medium">Total Unpaid</span><span class="font-bold text-white text-base tabular-nums">${formatCurrency(currentTotal)}</span>`;
        } else if (searchTerm) {
          footerDiv.innerHTML = `<span class="text-[var(--text-secondary)] font-medium">Search Results (${filteredTransactions.length})</span><span class="font-bold text-white text-base">Total: <span class="tabular-nums">${formatCurrency(currentTotal)}</span></span>`;
        } else {
          footerDiv.innerHTML = `<span class="text-[var(--text-secondary)] font-medium">Monthly Total</span><span class="font-bold text-white text-base tabular-nums">${formatCurrency(currentTotal)}</span>`;
        }
  ...
  window.openBulkPayCcItemForm = () => {
    ...
    window.ccSelectedItems.forEach(id => {
      ...
      if (item) {
        ...
        itemsHtml += `
          <div class="mb-3 pb-3 border-b border-[var(--border-color)] last:border-0 last:pb-0">
    ...
    const formHtml = `
        ...
        <p class="mb-4 text-sm text-[var(--text-secondary)]">You are settling <span class="text-white font-semibold">${window.ccSelectedItems.size} items</span> for a total of <span class="font-semibold text-expense text-base tabular-nums">${formatCurrency(totalAmount)}</span>.</p>
        <div class="mb-4">
        ...
        <div class="mb-4 mt-5 max-h-[40vh] overflow-y-auto custom-scrollbar pr-2">
          <label class="block text-sm font-medium mb-3 text-[var(--text-secondary)]">Categorize Items</label>
          ${itemsHtml}
        </div>
        
        <p class="text-xs text-[var(--text-secondary)] mb-4 bg-tertiary border-[var(--border-color)] p-3 rounded"><i class="fas fa-info-circle mr-1"></i> A separate transaction will be created in your bank account for each item to ensure pie charts remain accurate.</p>
  ...
  const formHtml = `
            ...
            <div>
                ...
                <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="modalCcAmount" name="ccAmount" value="${transaction.amount.toFixed(
                  2
                )}" step="0.01" min="0" placeholder="Amount spent" required><button type="button" class="calc-toggle-btn absolute right-4 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div>
            </div>
            <div>
  ...
  const formHtml = `
      ...
      <div>
          ...
          <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="payDebtAmount" name="payDebtAmount" step="0.01" min="0.01" max="${debt.remainingAmount.toFixed(
            2
          )}" value="${debt.remainingAmount.toFixed(2)}" required><button type="button" class="calc-toggle-btn absolute right-2 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div>
      </div>
      <div>
      ...
      <div class="flex items-center mt-3 mb-1">
          <input type="checkbox" id="logDebtPaymentAsExpense" name="logDebtPaymentAsExpense" class="h-4 w-4 text-accent-500 border-[var(--border-color)] rounded focus:ring-accent-500 mr-2" checked>
          <label for="logDebtPaymentAsExpense" class="text-sm font-medium text-[var(--text-secondary)]">Log this payment as an expense?</label>
      </div>
...
function openReceivePaymentForm(recId) {
  ...
  const overpaymentInfoHtml = `
    <p id="overpaymentInfo" class="text-xs text-[var(--text-secondary)] mt-1 mb-2" style="display: none;">
  ...
  const formHtml = `
    ...
    <div>
      ...
      <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="recPaymentAmount" name="recPaymentAmount" step="0.01" min="0.01" 
             value="${receivable.remainingAmount.toFixed(2)}" required><button type="button" class="calc-toggle-btn absolute right-2 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div> 
    </div>
    ${overpaymentInfoHtml}
    <div>
  ...
  const formHtml = `
    ...
    <div>
      ...
      <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="instFullAmount" name="instFullAmount" value="${currentFullAmount.toFixed(
        2
      )}" step="0.01" min="0.01" required><button type="button" class="calc-toggle-btn absolute right-2 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div>
    </div>
    <div>
      <label for="instTotalMonths" class="block text-sm font-medium mb-1">Total Months for Plan</label>
      <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="instTotalMonths" name="instTotalMonths" value="${
        i.totalMonths
      }" step="1" min="1" required><button type="button" class="calc-toggle-btn absolute right-2 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div>
    </div>
    <div>
      ...
      <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="instMonthsLeft" name="instMonthsLeft" value="${
        i.monthsLeft
      }" step="1" min="0" max="${i.totalMonths}" required><button type="button" class="calc-toggle-btn absolute right-2 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div>
    </div>
    <div>
      <label for="instStartDate" class="block text-sm font-medium mb-1">Start Date</label>
...
function payInstallmentMonth(installmentId) {
  ...
  const confirmationMessageHtml = `
    <p class="mb-4 text-center text-[var(--text-secondary)] force-word-wrap">
      Mark one month as paid for "<strong>${installment.description}</strong>"?
    </p>
    <p class="mb-4 text-center text-sm text-[var(--text-secondary)]">
  ...
  const formHtml = `
      ...
      <div>
          ...
          <div class="relative flex items-center w-full"><input type="text" inputmode="decimal" class="calc-amount pr-8" id="ccItemPayAmount" name="ccItemPayAmount" step="0.01" min="0.01" max="${remaining.toFixed(
            2
          )}" value="${remaining.toFixed(2)}" required><button type="button" class="calc-toggle-btn absolute right-2 text-[var(--text-secondary)] hover:text-accent-500 transition-colors focus:outline-none" tabindex="-1"><i class="fas fa-calculator"></i></button></div>
      </div>
      <div>
      ...
      <div class="flex items-center mt-3 mb-1">
          <input type="checkbox" id="logCcPaymentAsExpense" name="logCcPaymentAsExpense" class="h-4 w-4 text-accent-500 border-[var(--border-color)] rounded focus:ring-accent-500 mr-2" checked>
          <label for="logCcPaymentAsExpense" class="text-sm font-medium text-[var(--text-secondary)]">Log this payment as an expense?</label>
      </div>
  ...
  if (debtsInstallmentsCard) {
    if (hideDebts && hideInstallments) {
      ...
      if (hideDebts) {
        ...
        if (installmentsContainer) installmentsContainer.classList.remove("mt-6", "pt-4", "border-t", "border-[var(--border-color)]");
        ...
        if (installmentsContainer) installmentsContainer.classList.add("mt-6", "pt-4", "border-t", "border-[var(--border-color)]");
  ...
  if (shouldShowAccountCards) {
    ...
    if (!listsMatch) {
      ...
      visibleAccounts.forEach((acc) => {
        ...
        card.className = "bg-[var(--bg-tertiary)] border border-[var(--border-color)] p-3 rounded";
        card.innerHTML = `
          <p class="text-xs font-medium text-[var(--text-secondary)] truncate">${acc.name}</p>
  ...
  if (isCollapsed) {
    ...
    if (headerEl) {
      if (showTip) {
        ...
        headerEl.classList.add("border-b", "border-[var(--border-color)]", "pb-2", "mb-4");
      } else {
        headerEl.classList.remove("border-b", "border-[var(--border-color)]", "pb-2", "mb-4");
  ...
  if (tipContainer) {
    if (showTip) {
      ...
      tipContainer.innerHTML = `
        <div class="flex items-start justify-between bg-accent-500/10 border border-accent-500/30 rounded-lg p-3 text-xs text-[var(--text-secondary)] relative">
          ...
          <button id="dismissBudgetTipBtn" class="text-[var(--text-secondary)] hover:text-white ml-2 focus:outline-none" data-tooltip="Dismiss">
    ...
    toggleBtn.addEventListener("click", () => {
      ...
      if (currentlyCollapsed) {
        ...
        if (header) {
          ...
          header.classList.add("border-b", "border-[var(--border-color)]", "pb-2", "mb-4");
        }
        ...
        const onTransitionEnd = () => {
          if (localStorage.getItem("kaasi_collapseCategoryBudgets") === "true") {
            if (header && !isTipVisible) {
              header.classList.remove("border-b", "border-[var(--border-color)]", "pb-2", "mb-4");
  ...
  const updateFn = () => {
    ...
    if (isNew) {
      ...
      budgetItem.innerHTML = `
        ...
        <div class="w-full bg-[var(--bg-tertiary)] border border-[var(--border-color)] rounded-full h-2 overflow-hidden cursor-pointer hover:opacity-80 transition-opacity budget-progress-container">
    ...
    if (statusEl) {
      if (!budget.categories || budget.categories.length === 0) {
        ...
        statusEl.className = `text-xs font-semibold budget-status whitespace-nowrap ${isOver ? 'text-[#E74C3C]' : 'text-[var(--text-secondary)]'}`;
