...
async function restoreFromSupabase(force = false, isFromDashboard = false) {
  ...
  const doRestore = async () => {
    ...
    if (force) {
      ...
      let extraHtml = "";
      if (isFromDashboard) {
        extraHtml = `<div class="mt-4 pt-4 border-t border-[var(--border-color)]">
          <label class="flex items-start gap-2.5 cursor-pointer group text-left max-w-[260px] mx-auto">
            ...
            <div class="shrink-0 w-4 h-4 rounded border border-[var(--border-color)] peer-checked:border-accent-500 flex items-center justify-center transition-colors text-transparent peer-checked:text-accent-500 mt-[3px]">
            ...
            <span class="text-sm text-[var(--text-secondary)] group-hover:text-white transition-colors leading-[1.35]">Don't show this warning again when restoring from the dashboard</span>
          </label>
        </div>`;
      }
...
function updateSupabaseUI(user) {
  ...
  if (user) {
    ...
    shortcutCloudLabel.classList.add("text-[var(--text-secondary)]");
    ...
    shortcutCloudLabel.classList.remove("text-[var(--text-secondary)]");
  }
