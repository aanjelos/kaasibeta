...
window.hasDismissedOfflineWarning = false;

/**
 * Utility to verify if the server is actually reachable, solving navigator.onLine issues on desktops.
 */
async function checkRealOnlineStatus() {
  console.log("[OfflineCheck] Running checkRealOnlineStatus. navigator.onLine:", navigator.onLine);
  if (!navigator.onLine) return false;
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 2500);
    // Ping SUPABASE_URL which is globally defined in globals.js
    console.log("[OfflineCheck] Fetching SUPABASE_URL:", SUPABASE_URL);
    await fetch(SUPABASE_URL, { method: "GET", mode: "no-cors", signal: controller.signal });
    clearTimeout(timeoutId);
    console.log("[OfflineCheck] Fetch succeeded. Real online status: true");
    return true;
  } catch (err) {
    console.warn("[OfflineCheck] Fetch failed. Real online status: false", err);
    return false;
  }
}
 ...
 * If so, displays a warning modal about sync issues.
 */
window.checkOfflineSyncWarning = async function() {
  console.log("[OfflineCheck] checkOfflineSyncWarning called. Sync method:", localStorage.getItem("preferredSyncMethod"));
  if (localStorage.getItem("preferredSyncMethod") === "cloud") {
    const isOnline = await checkRealOnlineStatus();
    console.log("[OfflineCheck] checkOfflineSyncWarning result. isOnline:", isOnline);
    if (!isOnline) {
      const modal = $("#offlineSyncWarningModal");
      if (modal) {
        console.log("[OfflineCheck] Displaying warning modal.");
        modal.style.display = "block";
        if (typeof updateBodyScrollState === "function") {
          updateBodyScrollState();
        }
      } else {
        console.error("[OfflineCheck] Modal element not found!");
      }
    }
  }
};
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (continueBtn && modal) {
    continueBtn.addEventListener("click", () => {
      window.hasDismissedOfflineWarning = true;
      modal.style.display = "none";
      if (typeof updateBodyScrollState === "function") {
        updateBodyScrollState();
      }
...
window.addEventListener("online", async () => {
  console.log("[OfflineCheck] Native online event triggered.");
  const isOnline = await checkRealOnlineStatus();
  if (isOnline) {
    window.hasDismissedOfflineWarning = false;
    showNotification("Back online! Connection restored.", "success");
...
window.addEventListener("offline", () => {
  console.log("[OfflineCheck] Native offline event triggered.");
  if (localStorage.getItem("preferredSyncMethod") === "cloud" && !window.hasDismissedOfflineWarning) {
    const modal = $("#offlineSyncWarningModal");
    if (modal) {
      modal.style.display = "block";
      if (typeof updateBodyScrollState === "function") {
...
// Periodic background check for offline status (checks every 15s to bypass unreliable desktop offline events)
setInterval(async () => {
  console.log("[OfflineCheck] Polling check running... visibilityState:", document.visibilityState, "SyncMethod:", localStorage.getItem("preferredSyncMethod"));
  if (document.visibilityState === "visible" && localStorage.getItem("preferredSyncMethod") === "cloud") {
    const isOnline = await checkRealOnlineStatus();
    const modal = $("#offlineSyncWarningModal");
    
    if (!isOnline) {
      if (!window.hasDismissedOfflineWarning && modal && modal.style.display !== "block") {
        console.log("[OfflineCheck] Polling detected offline, showing modal.");
        modal.style.display = "block";
        if (typeof updateBodyScrollState === "function") {
          updateBodyScrollState();
        }
      ...
      // Reconnected
      if (window.hasDismissedOfflineWarning || (modal && modal.style.display === "block")) {
        console.log("[OfflineCheck] Polling detected online, hiding modal.");
        window.hasDismissedOfflineWarning = false;
        if (modal && modal.style.display === "block") {
          modal.style.display = "none";
          if (typeof updateBodyScrollState === "function") {
